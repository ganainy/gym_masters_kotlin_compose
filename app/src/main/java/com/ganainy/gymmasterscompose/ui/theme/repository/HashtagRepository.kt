...
import com.ganainy.gymmasterscompose.ui.theme.models.Hashtag.Companion.HASHTAGS
...
class HashtagRepository @Inject constructor(
    ...
    private suspend fun loadHashTagData(tags: List<String>): List<Hashtag> {
        return try {
            val tagsRef = database.getReference(HASHTAGS)
    ...
    override suspend fun updateHashtags(tags: List<String>): Map<String, Any> {
        ...
        if (tags.isNotEmpty()) {
            ...
            existingHashTags.forEach { hashtag ->
                updates["$HASHTAGS/${hashtag.tag}/$HASHTAG_USE_COUNT"] = ServerValue.increment(1)
                updates["$HASHTAGS/${hashtag.tag}/$HASHTAG_LAST_USED"] = System.currentTimeMillis()
            }

            // Create new hashtags
            tags.filterNot { it in existingTagNames }.forEach { newTag ->
                updates["$HASHTAGS/$newTag"] = newTag.toHashtag()
            }
        }
